...
export default function Login() {
  ...
  const captureAndLogin = async () => {
    ...
    try {
      ...
      if (result.success) {
        setMessage({ text: `✅ Login successful! Match: ${result.similarity}%`, type: 'success' });

        // ──── Email ko localStorage me save karo (dashboard ke liye) ────
        localStorage.setItem('scanpass_user', email);
        localStorage.setItem('scanpass_similarity', result.similarity);
        localStorage.setItem('scanpass_loginTime', new Date().toISOString());

        // 1.5 second baad dashboard pe le jao
      ...
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="glass-card p-8 w-full max-w-md page-enter">
          ...
          <div className="text-center mb-6">
            ...
            <h1 className="text-3xl font-bold glow-text">ScanPass Login</h1>
            <p className="text-sm mt-2" style={{ color: 'rgba(255,255,255,0.5)' }}>
              Scan your registered object to login
            </p>
          </div>
            ...
            {!scanning ? (
              ...
              <div>
                ...
                <div className="flex items-center justify-center mb-4 text-sm" style={{ color: 'rgba(255,255,255,0.6)' }}>
                  ...
                  Camera Active — Show your registered object
                </div>

                <button onClick={captureAndLogin} className="btn-success" disabled={loading}>
                  {loading ? (
          ...
          <p className="text-center text-sm fade-in fade-in-delay-3" style={{ color: 'rgba(255,255,255,0.5)' }}>
            Not registered?{' '}
            <a href="/" className="link">Register here →</a>
          </p>
        </div>
      </div>
    </div>
